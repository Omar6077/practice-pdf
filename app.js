...
const defaultWritingPages = $('defaultWritingPages');
const lineSpacing = $('lineSpacing');
const pageMargin = $('pageMargin');
const addMarginLine = $('addMarginLine');
const pageOverrides = $('pageOverrides');
const pageOverrideList = $('pageOverrideList');
const clearOverrides = $('clearOverrides');
...
function clampWritingPageCount(value, fallback = 1) {
  const parsed = Number.parseInt(value, 10);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.max(0, Math.min(10, parsed));
}

function defaultWritingPageCount() {
  return clampWritingPageCount(defaultWritingPages.value, 1);
}

function writingPageCountFor(index) {
  const override = pageWritingOverrides[index];
  return override === null || override === undefined
    ? defaultWritingPageCount()
    : clampWritingPageCount(override, defaultWritingPageCount());
}

function totalWritingPageCount() {
  if (!addLined.checked || !selectedPageCount) return 0;
  let total = 0;
  for (let index = 0; index < selectedPageCount; index += 1) total += writingPageCountFor(index);
  return total;
}

function updateOverridePlaceholders() {
  const fallback = String(defaultWritingPageCount());
  for (const input of pageOverrideList.querySelectorAll('input[data-page-index]')) input.placeholder = fallback;
}

function renderPageOverrides() {
  pageOverrideList.replaceChildren();
  pageOverrides.hidden = !selectedPageCount;
  if (!selectedPageCount) return;

  const fragment = document.createDocumentFragment();
  for (let index = 0; index < selectedPageCount; index += 1) {
    const row = document.createElement('label');
    row.className = 'page-override-row';

    const label = document.createElement('span');
    label.className = 'page-override-label';
    const strong = document.createElement('strong');
    strong.textContent = `PDF page ${index + 1}`;
    const small = document.createElement('small');
    small.textContent = 'Writing pages after this page';
    ...
    input.setAttribute('aria-label', `Writing pages after PDF page ${index + 1}`);
    input.placeholder = String(defaultWritingPageCount());
    input.value = pageWritingOverrides[index] ?? '';
    input.addEventListener('input', () => {
      const raw = input.value.trim();
      pageWritingOverrides[index] = raw === '' ? null : clampWritingPageCount(raw, defaultWritingPageCount());
      estimatePages();
    });
    input.addEventListener('blur', () => {
      if (input.value.trim() !== '') {
        const count = clampWritingPageCount(input.value, defaultWritingPageCount());
        pageWritingOverrides[index] = count;
  ...
  pageOverrideList.appendChild(fragment);
}
...
function estimatePages() {
  ...
  if (addLined.checked) total += totalWritingPageCount();
  if (addTitle.checked) {
    total += 1;
...
function syncOptionVisibility() {
  ...
  clearOverrides.disabled = !addLined.checked || working;
...
async function readSelectedFile(file) {
  ...
  try {
    ...
    pageWritingOverrides = Array(selectedPageCount).fill(null);
    renderPageOverrides();
...
async function createBooklet() {
  ...
  try {
    ...
    for (let index = 0; index < copiedPages.length; index += 1) {
      ...
      if (addLined.checked) {
        const writingPages = writingPageCountFor(index);
        for (let writingIndex = 0; writingIndex < writingPages; writingIndex += 1) {
      ...
      setProgress(percent, `Adding writing pages… PDF page ${index + 1} of ${copiedPages.length}`);
...
defaultWritingPages.addEventListener('input', () => {
  updateOverridePlaceholders();
  estimatePages();
});
defaultWritingPages.addEventListener('blur', () => {
  defaultWritingPages.value = String(defaultWritingPageCount());
  updateOverridePlaceholders();
  estimatePages();
});
